...
public class ClusteringCoefficientMetric implements NetworkMetric {
     ...
     *
     */
    class ClusteringCoefficientResult implements NetworkMetricResult {

        String[] mNames = {"Clustering Coefficient"};
        double[] mValues;

        /**
         *
         */
        ClusteringCoefficientResult(double pCC) {
            mValues = new double[1];
            mValues[0] = pCC;
        }
    ...
    public NetworkMetricResult analyze(RandomNetwork pNetwork, boolean pDirected) {
       ...
       // IntEnumerator nodeIterator = pNetwork.nodes();
        Set<Integer> nodes = pNetwork.nodes();

        //Use as the number of nodes in the network
        int N = nodes.size();//nodeIterator.numRemaining();

        //Use this to temo
        List<Integer> nodeInRep[] = new ArrayList[N];
        List<Integer> nodeOutRep[] = new ArrayList[N];
        ...
        for (int i = 0; i < N; i++) {
            ...
            List<Integer> neighborhood = new ArrayList<Integer>(nodeOutRep[i]);
            ...
            while (neighborhood.size() > 0) {
                ...
                int neighbor1 = (Integer) neighborhood.remove(0);
        ...
        return new ClusteringCoefficientResult(averageClusteringCoefficient / N);
    }
}
