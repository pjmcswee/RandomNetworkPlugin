    ...
    public RandomNetworkImpl generate() {
        ...
        for (int i = 0; i < init_num_nodes; i++) {
            for (int j = (i + 1); j < init_num_nodes; j++) {
                ...
                increment(degrees, nodes[i]);
                increment(degrees, nodes[j]);
                //degrees.[i]++;
                //degrees[j]++;
        ...
        for (int i = init_num_nodes; i < numNodes; i++) {
            ...
            for (int m = 0; m < edgesToAdd; m++) {
                //keep a running tally of the probability
                double prob = 0;
                ...
                for (int j = 0; j < i; j++) {
                    ...
                    Set<Integer> connectingEdges = random_network.edgesConnecting(nodes[i], nodes[j], directed, false, !directed);
                    if (connectingEdges.isEmpty()) {
                        //Increment the talley by the jth node's probability
                        prob += (double) ((double) degrees.get(nodes[j]))
                    ...
                    if (randNum <= prob) {
                        ...
                        degreeIgnore += degrees.get(nodes[j]);
